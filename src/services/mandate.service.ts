...
export function isStorageNotConfigured(e: unknown): boolean {
  return has503Code(e, 'STORAGE_NOT_CONFIGURED');
}

/** True when the backend returned 503 SCANNER_UNAVAILABLE (virus scanner down — transient). */
export function isScannerUnavailable(e: unknown): boolean {
  return has503Code(e, 'SCANNER_UNAVAILABLE');
}
