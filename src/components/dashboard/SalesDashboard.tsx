    ...
    <>
      ...
      <div>
        ...
        <div style={{ marginBottom: 'var(--space-24)', ...anim(0) }}>
          ...
          {/* Inline Win Rate / Active Deals / Avg Velocity removed — they duplicated
              the KPI cards below one-for-one (audit P-05). Cards are the single
              canonical KPI presentation. */}
        </div>

        {/* ── KPI Cards ─────────────────────────────────────────── */}
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4" style={{ marginBottom: 'var(--space-16)', ...anim(200) }}>
          {/* Trend badges removed — they were hardcoded ('+4%', '-2 days', '+12%')
              with no prior-period data behind them (audit P-01). Re-add per-card
              `trend`/`trendValue` only once the metrics endpoint returns a real
              comparison window. */}
          {([
            { title: 'Win Rate',       value: `${metrics.winRate}%` },
            { title: 'Active Deals',   value: metrics.activeDeals,                                     subtitle: 'In pipeline' },
            { title: 'Avg Velocity',   value: `${metrics.avgVelocity} days` },
            { title: 'Pipeline Value', value: formatCurrency(metrics.pipelineValue, metrics.currency) },
          ] as const).map((card, i) => (
