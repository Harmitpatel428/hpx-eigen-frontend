...
import { parseCSV, parseTSV, IMPORTABLE_FIELDS, type ParseResult } from '../../utils/csv';
import { buildAutoMap, validateImportRows, rowToPayload, type RowError } from '../../utils/lead-import';
...
export function LeadImportWizard({ onClose, fieldDefs }: Props) {
  ...
  const handleText = useCallback((text: string, fromClipboard = false) => {
    setParseError(null);
    // Clipboard tables from Excel / Google Sheets arrive tab-separated; files
    // are comma-separated. Both go through the same quote-aware parser.
    const result = fromClipboard && text.includes('\t') ? parseTSV(text) : parseCSV(text);
    if (result.error) { setParseError(result.error); return; }
    if (result.headers.length === 0) { setParseError('No valid data found.'); return; }

    const autoMap = buildAutoMap(result.headers, fieldDefs);
  ...
  const handleFile = (file: File) => {
    if (!file.name.endsWith('.csv') && file.type !== 'text/csv') {
      setParseError('Please upload a CSV file. To bring in data from Excel or Google Sheets, use "Paste" below.');
      return;
    }
  ...
  const goToPreview = () => {
    if (!parsed) return;
    const errors = validateImportRows(parsed, columnMap);
      ...
      <div style={{ position: 'relative', zIndex: 1, background: '#fff', borderRadius: '0.875rem', boxShadow: '0 32px 64px -12px rgba(0,0,0,0.4)', width: '100%', maxWidth: 680, maxHeight: '92vh', overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 1.5rem', borderBottom: '1px solid #f1f5f9', position: 'sticky', top: 0, background: '#fff', zIndex: 10 }}>
          <div>
            ...
            <p style={{ fontSize: 11, color: '#94a3b8', marginTop: 1 }}>Import a CSV file, or paste data from Excel / Google Sheets</p>
          </div>
              ...
              {!pasteMode ? (
                ...
                <>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <button
                      type="button"
                      onClick={() => handleText(pasteText, true)}
